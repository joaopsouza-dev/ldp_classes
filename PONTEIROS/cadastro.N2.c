...
void case5(Alunos *ptr_aluno, int *qtd_alunos) {
    ...
    for(i = 0; i < *qtd_alunos; i++) {
        ...
        if(matricula == ptr_aluno[i].matricula) {
			...
			// isso aqui e so pra recalcular a media pra ficar certa e de acordo com a nova nota

            printf("\nMedia atualizada com sucesso!\n");
            printf("Nova media: %.2f\n", ptr_aluno[i].media);

            return; // isso aqui eu usei o chat, porque quando eu estava fazendo, mesmo encontrando o aluno pela matricula, ele simplesmente escrevia junto "matricula nao encontrada"
        }
    }
